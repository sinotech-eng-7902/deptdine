...
(() => {
  ...
  function activeSurveyLabel(){return D.surveys.find(item=>item.id===activeSurveyId)?.title||activeSurveyId||''}
  ...
  function ensureBanner(){
    let banner=document.getElementById('viewRoleBannerV1009');
    const main=document.querySelector('#admin .main');
    if(!main)return null;
    if(!banner){
      banner=document.createElement('div');banner.id='viewRoleBannerV1009';banner.className='viewRoleBannerV1009';
      banner.innerHTML='<div><b></b><span></span></div><button class="btn" type="button">返回系統管理員</button>';
      banner.querySelector('button').addEventListener('click',()=>switchViewRole('system'));
  ...
  function syncViewRoleUI(){
    ...
    if(banner){
      banner.hidden=!actualSystemAdmin||selectedRole==='system';
      banner.querySelector('b').textContent=`目前以${activeRoleLabel()}身分操作`;
      banner.querySelector('span').textContent=activeSurveyLabel()?`僅顯示：${activeSurveyLabel()}`:'僅顯示被分享的活動';
    }
