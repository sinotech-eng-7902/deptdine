...
function pricingSummaryV712(rest){
  ...
  let extras=[];if(numericV712(rest?.serviceRate,0))extras.push('另加服務費 '+numericV712(rest.serviceRate,0)+'%');if(numericV712(rest?.fixedFee,0))extras.push('另加固定費 '+moneyText(rest.fixedFee)+' 元');return base+(extras.length?'；'+extras.join('、'):'');
}
