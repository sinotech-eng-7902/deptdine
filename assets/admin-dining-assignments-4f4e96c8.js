/* v10.49 聚餐負責人安排：儀表板聚焦一筆並自動標示辦理狀態。 */
(()=>{
  'use strict';
  const VERSION='10.49';
  let editingId='';
  ...
  function render(){
    ...
    const items=normalizedItems(),visible=items.filter(mine),focused=focusItem(visible);
    const manage=document.getElementById('manageDiningAssignmentsV1046');if(manage)manage.hidden=!canManageAssignments();
    if(!focused){box.innerHTML='<div class="diningAssignmentEmptyV1046">目前尚未安排您負責的聚餐。</div>';return}
