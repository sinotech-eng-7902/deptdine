...
(function(){
  ...
  function applyActionsV900(){
    document.querySelectorAll('#surveyTable tbody tr[data-survey-id]').forEach(row=>{
      ...
      if(canManageRow)group.appendChild(makeBtnV900(archived?'恢復':'結案',archived?'btn surveyRestoreActionV1023':'btn surveyArchiveActionV1023',()=>window.archiveSurveyV775(s.id,!archived)));
      if(canDeleteSurveyV900(s))group.appendChild(makeBtnV900('刪除','btn red',()=>showDeleteSurveyModalV776(s.id)));
      if(!current&&!archived)group.appendChild(makeBtnV900('設目前','btn surveyCurrentActionV1023 currentActionV787',()=>setActiveSurvey(s.id)));
      ops.innerHTML='';
      ops.appendChild(group);
    });
  }
  function renderSurveyListV900(){
    ...
    box.classList.toggle('surveyTableSystemV1023',systemLayout);
    box.classList.toggle('surveyTableStandardV1023',!systemLayout);
