...
function targetMembers(){let targets=targetDepartments();return D.members.filter(m=>m.active!==false&&memberCanFill(m)&&(!targets.length||targets.includes(memberDepartment(m))))}
...
function renderOptions(){
  ...
  ui.dateBox.innerHTML=D.dates.length?`<div class="dateInfo">請勾選所有可以出席的日期。未勾選的日期將視為無法出席；若最終選定該日期，您將不列入出席名單。</div><div class="dateGrid">${D.dates.map(x=>`<label class="dateChoice"><input class="dateOpt" type="checkbox" value="${escAttr(x.id)}"><span>${esc(x.label)}</span></label>`).join('')}</div>`:'<div class="notice">尚未設定日期選項</div>';
...
function currentMemberByEmailV806(email){
  ...
  if(account){
    ...
    if(member&&member.active!==false)return member;
  }
  return D.members.find(m=>m.active!==false&&memberGoogleEmailV806(m)===normalized)||null;
}
