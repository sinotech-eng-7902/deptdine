/* v10.46 聚餐負責人安排：所有後台角色可檢視，僅系統管理員可維護。 */
(()=>{
  'use strict';
  const VERSION='10.46';
  let filter='mine';
  ...
  function period(item){return`${Number(item.startMonth)||''}至${Number(item.endMonth)||''}月辦理`}
  function normalizedItems(){
    return (Array.isArray(D?.diningAssignments)?D.diningAssignments:[]).map((item,index)=>({
      ...
      .sort((a,b)=>b.year-a.year||a.occurrence-b.occurrence);
  }
  function currentMemberId(){
    const current=email(currentUser?.email);
    const account=(D?.memberAccounts||[]).find(item=>email(item.email)===current);
    if(account)return String(account.memberId||account.id||'');
    return String((D?.members||[]).find(item=>email(item.googleEmail||item.email)===current)?.id||'');
  }
  function mine(item){
    const current=email(currentUser?.email),memberId=currentMemberId();
    return !!((current&&item.responsibleEmails.includes(current))||(memberId&&item.responsibleIds.includes(memberId)));
  }
  function render(){
    const box=document.getElementById('diningAssignmentListV1046');if(!box)return;
    const items=normalizedItems(),visible=filter==='mine'?items.filter(mine):items;
    const manage=document.getElementById('manageDiningAssignmentsV1046');if(manage)manage.hidden=!isSystemAdmin;
    document.getElementById('diningAssignmentMineV1046')?.classList.toggle('active',filter==='mine');
    document.getElementById('diningAssignmentAllV1046')?.classList.toggle('active',filter==='all');
    if(!visible.length){box.innerHTML=`<div class="diningAssignmentEmptyV1046">${filter==='mine'?'目前尚未安排您負責的聚餐。可切換「查看全部」了解各次安排。':'目前尚未建立聚餐負責人安排。'}</div>`;return}
    box.innerHTML=visible.map(item=>`<article class="diningAssignmentItemV1046${mine(item)?' isMine':''}"><div class="diningAssignmentItemTopV1046"><h4>${html(title(item))}</h4>${mine(item)?'<span class="diningAssignmentMineBadgeV1046">我負責</span>':''}</div><div class="diningAssignmentPeriodV1046">${html(period(item))}</div><div class="diningAssignmentPeopleV1046">${item.responsibleNames.map(name=>`<span>${html(name)}</span>`).join('')||'<span>尚未設定負責人</span>'}</div></article>`).join('');
  }
  function setFilter(value){filter=value==='all'?'all':'mine';render()}
  ...
  function renderAdminList(){
    ...
    box.innerHTML=items.map(item=>`<div class="diningAssignmentAdminRowV1046"><div><b>${html(title(item))}</b><small>${html(period(item))}</small></div><div>${html(item.responsibleNames.join('、')||'尚未設定負責人')}</div><div class="diningAssignmentAdminActionsV1046"><button class="btn" type="button" onclick="editDiningAssignmentV1046('${attr(item.id)}')">編輯</button><button class="btn red" type="button" onclick="deleteDiningAssignmentV1046('${attr(item.id)}')">刪除</button></div></div>`).join('')||'<div class="diningAssignmentEmptyV1046">目前尚未建立安排。</div>';
  }
  function openManager(){if(!isSystemAdmin)return alert('只有系統管理員可以管理聚餐負責人安排');const mask=ensureDialog();editingId='';document.getElementById('diningAssignmentEditorV1046').hidden=true;renderAdminList();mask.style.display='flex';document.body.style.overflow='hidden'}
  ...
  function startEdit(item=null){
    if(!isSystemAdmin)return alert('只有系統管理員可以管理聚餐負責人安排');ensureDialog();editingId=item?.id||'';
  ...
  async function save(){
    if(!isSystemAdmin)return alert('只有系統管理員可以管理聚餐負責人安排');
    ...
    const merged=items.filter(item=>item.id!==editingId).concat(next).sort((a,b)=>b.year-a.year||a.occurrence-b.occurrence),button=document.getElementById('saveDiningAssignmentV1046');button.disabled=true;
  ...
  async function remove(id){
    if(!isSystemAdmin)return alert('只有系統管理員可以管理聚餐負責人安排');const items=normalizedItems(),before=items.find(item=>item.id===String(id));if(!before)return;
