...
const DESCRIPTION_TEXT_COLORS=new Set(['default','brand','strong','alert','teal']);
function sanitizeRichHtml(value){
  ...
  [...template.content.querySelectorAll('*')].forEach(node=>{
    ...
    let colorToken=node.tagName==='SPAN'?String(node.getAttribute('data-text-color')||'').trim():'';
    ...
    if(node.tagName==='SPAN'){
      if(DESCRIPTION_TEXT_COLORS.has(colorToken))node.setAttribute('data-text-color',colorToken);
