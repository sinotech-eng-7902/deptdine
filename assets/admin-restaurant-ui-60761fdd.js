...
(() => {
  ...
  function ensureRestaurantNoteDialogV1034(){
    ...
    mask.innerHTML=`<section class="restaurantNoteDialogV1034" role="dialog" aria-modal="true" aria-labelledby="restaurantNoteTitleV1034">
      ...
      <div id="restaurantNoteContentV1034" class="restaurantNoteContentV1034"></div>
      <footer><button class="btn" type="button">關閉</button></footer>
    </section>`;
    document.body.appendChild(mask);
    mask.querySelector('.restaurantNoteCloseV1034')?.addEventListener('click',closeRestaurantNoteV1034);
    mask.querySelector('footer .btn')?.addEventListener('click',closeRestaurantNoteV1034);
  ...
  function showRestaurantNoteV1034(id){
    ...
    const note=restaurant?.internalNote||restaurant?.opsNote||'';
    if(!restaurant||!note)return;
    ...
    mask.querySelector('#restaurantNoteContentV1034').textContent=note;
    mask.hidden=false;
    mask.style.display='flex';
    document.body.classList.add('modalOpen');
    mask.querySelector('.restaurantNoteCloseV1034')?.focus();
  }
  ...
  function decorateRestaurantListV1034(){
    ...
    rows.forEach((row,index)=>{
      ...
      if(noteCell){
        ...
        noteCell.innerHTML=note?`<button class="btn restaurantNoteButtonV1034" type="button" onclick="showRestaurantNoteV1034('${escAttr(restaurant.id)}')">查看備註</button>`:'<span class="restaurantNoteEmptyV1034">—</span>';
      }
    });
  }
