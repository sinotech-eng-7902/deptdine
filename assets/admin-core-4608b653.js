...
function panel(id,b){if(['sysMemP','frontProtectP'].includes(id)&&!isSystemAdmin)return alert('此功能僅限系統管理員');if(id==='accessP'&&!canManage())return alert('此帳號沒有活動權限管理權限');if(['surveyP','memP','dateP','restP','costP','mailP','finalP'].includes(id)&&!canManage())return alert('此帳號只有檢視權限');let p=document.getElementById(id);if(!p)return;document.querySelectorAll('.panel').forEach(x=>x.classList.remove('active'));p.classList.add('active');document.querySelectorAll('.nav').forEach(x=>x.classList.remove('active'));if(b){b.classList.add('active');adminTitle.textContent=b.textContent}renderAdmin()}
