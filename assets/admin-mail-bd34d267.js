...
function renderMailRecipientListV957(type){
  ...
  if(rule)rule.textContent=mailTemplateV957(type).rule;
  if(warning){
  ...
  if(list){
    list.innerHTML=recipients.length
      ...
      :`<div class="mailRecipientEmptyV957">${type==='reminder'?'目前沒有尚未填寫且已設定公司信箱的人員。':'目前沒有可寄送且已設定公司信箱的人員。'}</div>`;
  }
...
function mailInfoCardV965(rows,theme,type,forEml=false,themeKey='reference'){
  ...
  const leftRule=type==='result'?`border-left:4px solid ${theme.labelColor};`:'';
...
function mailCorporateFooterV965(){
  return `${mailSpacerV967(16)}<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:collapse;mso-table-lspace:0pt;mso-table-rspace:0pt">
    <tr><td style="padding-top:7px;border-top:2px solid #1f4a77;color:#56748f;font-family:Arial,sans-serif;font-size:8pt;letter-spacing:.1em">DEPARTMENT DINING</td><td style="padding-top:7px;border-top:2px solid #1f4a77;text-align:right;color:#2a8b88;font-family:Arial,sans-serif;font-size:8pt">SINOTECH</td></tr>
  </table>`;
}
