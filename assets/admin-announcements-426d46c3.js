...
(() => {
  ...
  async function saveFinalDecision(){
    ...
    const historyApi=window.RestaurantHistoryV1030;
    if(historyApi?.buildSnapshot){
      if(finalDate.value&&finalRest.value){
        const historySnapshot=historyApi.buildSnapshot();
        if(historySnapshot){
          finalPayload.restaurantHistorySnapshot=historySnapshot;
          finalPayload.restaurantHistoryCapturedAt=firebase.firestore.FieldValue.serverTimestamp();
        }
      }else{
        finalPayload.restaurantHistorySnapshot=firebase.firestore.FieldValue.delete();
        finalPayload.restaurantHistoryCapturedAt=firebase.firestore.FieldValue.delete();
      }
    }
    try{
  ...
  window.saveFinalDecisionV1033=saveFinalDecision;
