/* v10.48 聚餐負責人安排：所有後台角色可檢視，僅未模擬身分的系統管理員可維護。 */
(()=>{
  'use strict';
  const VERSION='10.48';
  let filter='mine';
  ...
  function currentMemberId(){
    const simulatedId=simulatedMemberId();
    if(simulatedId)return simulatedId;
  ...
  function mine(item){
    const simulated=simulatedMember();
    const current=simulated
      ?email(typeof memberGoogleEmail==='function'?memberGoogleEmail(simulated):simulated.googleEmail||simulated.email)
      :email(currentUser?.email);
    const memberId=currentMemberId();
    return !!((current&&item.responsibleEmails.includes(current))||(memberId&&item.responsibleIds.includes(memberId)));
  }
  function itemMarkup(item){
    return `<article class="diningAssignmentItemV1046${mine(item)?' isMine':''}"><div class="diningAssignmentItemTopV1046"><h4>${html(title(item))}</h4>${mine(item)?'<span class="diningAssignmentMineBadgeV1046">我負責</span>':''}</div><div class="diningAssignmentMetaRowV1048"><b>辦理期間：</b><span>${html(period(item))}</span></div><div class="diningAssignmentMetaRowV1048 diningAssignmentPeopleRowV1048"><b>負責人：</b><span class="diningAssignmentPeopleTextV1048">${html(item.responsibleNames.join('、')||'尚未設定')}</span></div></article>`;
  }
  function render(){
    const box=document.getElementById('diningAssignmentListV1046');if(!box)return;
    const items=normalizedItems(),visible=items.filter(mine);
    const manage=document.getElementById('manageDiningAssignmentsV1046');if(manage)manage.hidden=!canManageAssignments();
    document.getElementById('diningAssignmentMineV1046')?.classList.add('active');
    document.getElementById('diningAssignmentAllV1046')?.classList.remove('active');
    if(!visible.length){box.innerHTML='<div class="diningAssignmentEmptyV1046">目前尚未安排您負責的聚餐。可點選「查看全部」了解各次安排。</div>';return}
    box.innerHTML=visible.map(itemMarkup).join('');
  }
  function ensureAllDialog(){
    ...
    mask.innerHTML='<div class="modal diningAssignmentDialogV1046 diningAssignmentAllDialogV1048"><div class="diningAssignmentDialogHeadV1046"><div><h2 id="diningAssignmentAllTitleV1048">全部聚餐負責人安排</h2><p>依年度與次別由新到舊顯示。</p></div><button class="diningAssignmentCloseV1046" type="button" aria-label="關閉" onclick="closeDiningAssignmentAllV1048()">×</button></div><div class="diningAssignmentDialogBodyV1046"><div id="diningAssignmentAllListV1048" class="diningAssignmentListV1046 diningAssignmentAllListV1048"></div></div></div>';
  ...
  function openAllDialog(){
    const mask=ensureAllDialog(),box=document.getElementById('diningAssignmentAllListV1048'),items=normalizedItems();
    box.innerHTML=items.length?items.map(itemMarkup).join(''):'<div class="diningAssignmentEmptyV1046">目前尚未建立聚餐負責人安排。</div>';
  ...
  function setFilter(value){filter='mine';if(value==='all')return openAllDialog();render()}
