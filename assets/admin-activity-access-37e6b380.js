...
(function installFinalAttendanceAdjustmentLayoutV783(){
  ...
  let finalAdjustModalOpenV1010=false;
  ...
  function effectiveAdjustmentSummaryV1010(){
    const items=[...latestAdjustmentMapV783().values()];
    return {
      count:items.length,
      added:items.filter(item=>item.action==='add').length,
      removed:items.filter(item=>item.action==='remove').length
    };
  }
  ...
  function removedAdjustmentRowsV783(dateId){
    ...
    return [...latestAdjustmentMapV783().values()]
  ...
  function unavailableRowsV783(dateId){
    const effective=latestAdjustmentMapV783();
    const manuallyAdded=new Set([...effective.values()].filter(item=>item.action==='add').map(item=>item.memberId));
    const fromResponses=unavailableResponsesForDate(dateId).filter(r=>!manuallyAdded.has(r.memberId)).map(r=>({
  ...
  function updateFinalAttendanceAdjustmentActionV1010(){
    const mount=document.getElementById('finalAttendanceAdjustmentActionV1010');
    if(!mount)return;
    const dateId=document.getElementById('finalDate')?.value||'';
    const summary=effectiveAdjustmentSummaryV1010();
    const statusTitle=`加入 ${summary.added} 人・移除 ${summary.removed} 人`;
    mount.innerHTML=`${summary.count?`<span class="finalAdjustmentStatusV1010" title="${escAttr(statusTitle)}"><span class="finalAdjustmentStatusDesktopV1010">人工調整 ${summary.count} 人</span><span class="finalAdjustmentStatusMobileV1010">已調整 ${summary.count} 人</span></span>`:''}<button class="btn finalAdjustmentButtonV1010" type="button" onclick="openFinalAttendanceAdjustmentV1010()" ${dateId?'':'disabled'} aria-describedby="finalAttendanceAdjustmentHintV1010">調整出席名單</button>${dateId?'':'<span id="finalAttendanceAdjustmentHintV1010" class="finalAdjustmentHintV1010">請先選定最終日期</span>'}`;
  }
  function ensureFinalAttendanceAdjustmentModalV1010(){
    let mask=document.getElementById('finalAttendanceAdjustmentModalV1010');
    if(mask)return mask;
    mask=document.createElement('div');
    mask.id='finalAttendanceAdjustmentModalV1010';
    mask.className='modalMask finalAttendanceAdjustmentModalV1010';
    mask.hidden=true;
    mask.innerHTML=`<div class="modal finalAttendanceAdjustmentDialogV1010" role="dialog" aria-modal="true" aria-labelledby="finalAttendanceAdjustmentTitleV1010">
      <div class="finalAttendanceAdjustmentModalHeadV1010">
        <div><h3 id="finalAttendanceAdjustmentTitleV1010">調整出席名單</h3><p id="finalAttendanceAdjustmentMetaV1010" class="muted"></p></div>
        <button class="iconBtn finalAttendanceAdjustmentCloseV1010" type="button" onclick="closeFinalAttendanceAdjustmentV1010()" aria-label="關閉調整出席名單視窗">×</button>
      </div>
      <div class="finalAttendanceAdjustmentModalBodyV1010"><p class="muted">此處只調整最終出席名單，不會修改同仁原始問卷內容。</p><div id="finalAttendanceAdjustmentEditorV1010"></div></div>
      <div class="finalAttendanceAdjustmentModalFootV1010"><button class="btn" type="button" onclick="closeFinalAttendanceAdjustmentV1010()">完成</button></div>
    </div>`;
    mask.addEventListener('click',event=>{if(event.target===mask)closeFinalAttendanceAdjustmentV1010()});
    document.body.appendChild(mask);
    return mask;
  }
  function renderFinalAttendanceAdjustmentModalV1010(){
    const mask=ensureFinalAttendanceAdjustmentModalV1010();
    const dateId=document.getElementById('finalDate')?.value||'';
    const date=D.dates.find(item=>item.id===dateId);
    const rows=finalAttendanceRowsV783(dateId);
    const editor=mask.querySelector('#finalAttendanceAdjustmentEditorV1010');
    const meta=mask.querySelector('#finalAttendanceAdjustmentMetaV1010');
    if(meta)meta.textContent=dateId?`${date?.label||'最終日期'}・目前最終出席 ${rows.length} 人`:'請先選定最終日期';
    if(editor)editor.innerHTML=dateId?renderAdjustmentEditorV783():'<div class="finalEmpty">請先選定最終日期後再調整出席名單。</div>';
  }
  function renderFinalAttendancePreviewV783(){
    ...
    updateFinalAttendanceAdjustmentActionV1010();
    if(!dateId){
      box.innerHTML='<div class="finalEmpty">請先選擇最終日期，系統將自動整理當天出席名單。</div>';
      return;
    }
    ...
    const unavailable=unavailableRowsV783(dateId),manuallyAddedIds=new Set([...latestAdjustmentMapV783().values()].filter(item=>item.action==='add').map(item=>item.memberId)),missing=missingMembers().filter(member=>!manuallyAddedIds.has(member.id));
    ...
    if(finalAdjustModalOpenV1010)renderFinalAttendanceAdjustmentModalV1010();
  }
  window.toggleFinalAttendanceAdjustmentV783=function(){
    if(finalAdjustModalOpenV1010)closeFinalAttendanceAdjustmentV1010();else openFinalAttendanceAdjustmentV1010();
  };
  window.openFinalAttendanceAdjustmentV935=function(){
    ...
    requestAnimationFrame(()=>openFinalAttendanceAdjustmentV1010());
  };
  window.openFinalAttendanceAdjustmentV1010=function(){
    const dateId=document.getElementById('finalDate')?.value||'';
    if(!dateId)return toast('請先選定最終日期');
    const mask=ensureFinalAttendanceAdjustmentModalV1010();
    finalAdjustModalOpenV1010=true;
    renderFinalAttendanceAdjustmentModalV1010();
    mask.hidden=false;
    mask.style.display='flex';
    document.body.classList.add('modalOpen');
    requestAnimationFrame(()=>mask.querySelector('#finalAdjustMemberV783')?.focus());
  };
  window.closeFinalAttendanceAdjustmentV1010=function(){
    const mask=document.getElementById('finalAttendanceAdjustmentModalV1010');
    finalAdjustModalOpenV1010=false;
  ...
  window.addFinalAttendanceAdjustmentV783=async function(){
    ...
    await loadSurveyData();renderFront();renderAdmin();
    if(finalAdjustModalOpenV1010)openFinalAttendanceAdjustmentV1010();
    toast('最終出席調整已新增');
  };
  ...
  function installFinalAdjustmentStylesV783(){
    ...
    style.textContent='.finalDecisionHeaderV1010{display:flex;align-items:flex-start;justify-content:space-between;gap:24px;margin-bottom:22px}.finalDecisionHeaderV1010 h3{margin:0 0 10px}.finalDecisionHeaderV1010 .muted{margin:0}.finalAttendanceAdjustmentActionV1010{display:flex;align-items:center;justify-content:flex-end;gap:10px;flex-wrap:wrap;min-width:240px}.finalAdjustmentButtonV1010{white-space:nowrap}.finalAdjustmentStatusV1010{display:inline-flex;align-items:center;min-height:34px;padding:6px 11px;border-radius:999px;background:#edf4fb;color:#315b84;font-size:14px;font-weight:700}.finalAdjustmentStatusMobileV1010{display:none}.finalAdjustmentHintV1010{flex-basis:100%;text-align:right;color:#71839a;font-size:13px}.finalAdjustmentButtonV1010:disabled{cursor:not-allowed;opacity:.56}.finalAttendanceAdjustmentModalV1010[hidden]{display:none!important}.finalAttendanceAdjustmentModalV1010{z-index:2500;padding:24px}.finalAttendanceAdjustmentDialogV1010{box-sizing:border-box;width:min(1180px,calc(100vw - 48px));max-width:1180px;max-height:calc(100vh - 48px);display:flex;flex-direction:column;overflow:hidden}.finalAttendanceAdjustmentModalHeadV1010{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;padding:24px 26px 18px;border-bottom:1px solid var(--line,#dbe4ef)}.finalAttendanceAdjustmentModalHeadV1010 h3{margin:0 0 6px}.finalAttendanceAdjustmentModalHeadV1010 .muted{margin:0}.finalAttendanceAdjustmentCloseV1010{font-size:26px;line-height:1}.finalAttendanceAdjustmentModalBodyV1010{padding:20px 26px;overflow:auto}.finalAttendanceAdjustmentModalBodyV1010>p{margin:0 0 16px}.finalAttendanceAdjustmentModalFootV1010{display:flex;justify-content:flex-end;padding:16px 26px;border-top:1px solid var(--line,#dbe4ef);background:#fff}.finalAdjustEditorV783{margin:0;min-width:0}.finalAdjustFormV783{display:grid;grid-template-columns:minmax(220px,1.2fr) 160px 170px minmax(180px,1fr) auto;gap:12px;align-items:end;margin:0 0 14px}.finalAdjustFormV783 .field{min-width:0}.finalAdjustButtonFieldV783 .btn{height:52px;white-space:nowrap;padding-left:22px;padding-right:22px}.finalAttendanceAdjustmentModalV1010 .finalAdjustButtonFieldV783 .btn.primary{border-color:transparent;background:#2563eb;color:#fff;box-shadow:0 5px 12px rgba(37,99,235,.18)}.finalAttendanceAdjustmentModalV1010 .finalAdjustButtonFieldV783 .btn.primary:hover:not(:disabled){border-color:transparent;background:#1d4ed8;color:#fff;box-shadow:0 8px 18px rgba(29,78,216,.24);transform:translateY(-1px)}.finalAttendanceAdjustmentModalV1010 .finalAdjustButtonFieldV783 .btn.primary:active:not(:disabled){background:#1e40af;color:#fff;box-shadow:0 3px 8px rgba(30,64,175,.24);transform:translateY(0)}.finalAttendanceAdjustmentModalV1010 .finalAdjustButtonFieldV783 .btn.primary:disabled{background:#dbe3ed;color:#64748b;box-shadow:none;transform:none}.finalAdjustEditorV783 .tableWrap{width:100%;max-width:100%;overflow-x:auto}.finalAdjustEditorV783 .table{width:100%;margin-top:10px}.finalAdjustEditorV783 .table th{white-space:nowrap}.finalAdjustEditorV783 .table td{word-break:normal}.finalAdjustEditorV783 .table td:nth-child(1),.finalAdjustEditorV783 .table td:nth-child(2),.finalAdjustEditorV783 .table td:nth-child(3),.finalAdjustEditorV783 .table td:nth-child(4),.finalAdjustEditorV783 .table td:nth-child(9){white-space:nowrap}@media(max-width:1100px){.finalAdjustFormV783{grid-template-columns:1fr 1fr}.finalAdjustButtonFieldV783{grid-column:1 / -1}.finalAdjustButtonFieldV783 .btn{width:100%}}@media(max-width:700px){.finalDecisionHeaderV1010{flex-direction:column;gap:16px}.finalAttendanceAdjustmentActionV1010{width:100%;min-width:0;justify-content:flex-start}.finalAdjustmentHintV1010{text-align:left}.finalAdjustmentStatusDesktopV1010{display:none}.finalAdjustmentStatusMobileV1010{display:inline}.finalAttendanceAdjustmentModalV1010{padding:0}.finalAttendanceAdjustmentDialogV1010{width:100vw;max-width:none;max-height:100dvh;height:100dvh;border-radius:0}.finalAttendanceAdjustmentModalHeadV1010{padding:18px 20px 14px}.finalAttendanceAdjustmentModalBodyV1010{padding:16px 18px}.finalAttendanceAdjustmentModalFootV1010{padding:14px 18px}.finalAdjustFormV783{grid-template-columns:1fr}}';
