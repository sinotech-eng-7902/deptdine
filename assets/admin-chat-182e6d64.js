...
(function(){
  ...
  let stableBottomControllerV1007=null;
  ...
  function stopConversationListenersV915(){
    ...
    stableBottomControllerV1007?.cancel();
  ...
  function createChatDomV915(){
    ...
    if(!document.getElementById('chatDrawerV915')){
      ...
      ['wheel','touchstart','pointerdown'].forEach(type=>drawer.querySelector('#chatMessageListV915').addEventListener(type,()=>stableBottomControllerV1007?.cancel(),{passive:true}));
      drawer.querySelector('#chatMessageListV915').addEventListener('keydown',event=>{if(['ArrowUp','PageUp','Home'].includes(event.key))stableBottomControllerV1007?.cancel()});
      ...
      stableBottomControllerV1007=window.AdminChatReadingPosition?.createStableBottomController?.({getList:()=>document.getElementById('chatMessageListV915'),onActiveChange:()=>syncJumpLatestV924()})||null;
    }
  ...
  function scrollChatToLatestV924(){const list=document.getElementById('chatMessageListV915');if(!list)return;visibleMessageLimitV925=50;pendingNewMessageCountV925=0;stableBottomControllerV1007?.start();renderMessagesV915();requestAnimationFrame(()=>list.scrollTo({top:list.scrollHeight,behavior:'smooth'}));document.getElementById('chatJumpLatestV924').hidden=true}
  function syncJumpLatestV924(){const list=document.getElementById('chatMessageListV915'),button=document.getElementById('chatJumpLatestV924');if(!list||!button)return;if(stableBottomControllerV1007?.isActive()){button.hidden=true;return}const near=list.scrollHeight-list.scrollTop-list.clientHeight<110;if(near)pendingNewMessageCountV925=0;button.textContent=pendingNewMessageCountV925?`${pendingNewMessageCountV925} 則新訊息`:'跳到最新訊息';button.hidden=near}
  ...
  function renderMessagesV915(){
    ...
    const stableBottomActive=stableBottomControllerV1007?.isActive()===true;
    const wasNearBottom=stableBottomActive||list.scrollHeight-list.scrollTop-list.clientHeight<110,previousTop=list.scrollTop;
    ...
    if(stableBottomControllerV1007?.isActive())stableBottomControllerV1007.touch();
    const scrollLatestAfterMedia=()=>{
      const apply=()=>{if(messageRenderGuardV1004&&!messageRenderGuardV1004.isCurrent(renderTokenV1004))return;list.scrollTop=list.scrollHeight;pendingNewMessageCountV925=0;syncJumpLatestV924()};
      apply();requestAnimationFrame(()=>{apply();requestAnimationFrame(apply)});
      list.querySelectorAll('img').forEach(image=>{if(!image.complete)image.addEventListener('load',apply,{once:true})});
    };
    requestAnimationFrame(()=>{if(messageRenderGuardV1004&&!messageRenderGuardV1004.isCurrent(renderTokenV1004))return;if(stableBottomControllerV1007?.isActive()){stableBottomControllerV1007.touch();forceScrollToBottomV924=false}else if(positionUnreadOnNextRenderV924&&window.AdminChatReadingPosition){window.AdminChatReadingPosition.positionMessageList(list,{mode:'unread',boundaryMessageId:unreadBoundaryMessageIdV924,anchorMessageId:lastReadAnchorMessageIdV1003});positionUnreadOnNextRenderV924=false}else if(forceScrollToBottomV924||wasNearBottom){if(forceScrollToBottomV924)stableBottomControllerV1007?.start();scrollLatestAfterMedia();forceScrollToBottomV924=false}else list.scrollTop=previousTop;if(query)focusSearchMatchV925(0);syncJumpLatestV924()});
  }
  ...
  function subscribeRoomV915(roomId){
    ...
    const acceptMessages=(snapshot,paged=true)=>{
      ...
      if(!initialReadingPositionResolvedV1003){
        ...
        positionUnreadOnNextRenderV924=position.mode==='unread';forceScrollToBottomV924=position.mode!=='unread';if(position.mode!=='unread')stableBottomControllerV1007?.start();initialReadingPositionResolvedV1003=true;
      }
