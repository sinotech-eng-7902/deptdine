...
(function(){
  ...
  function refreshSimulationToolbarV807(){
    ...
    select.value=D.members.some(m=>String(m.id)===String(simulateMemberIdV807))?simulateMemberIdV807:(current&&D.members.some(m=>String(m.id)===String(current))?current:'');
...
(function(){
  ...
  function applyActionsV900(){
    document.querySelectorAll('#surveyTable tbody tr[data-survey-id]').forEach(row=>{
      ...
      if(canManageRow)group.appendChild(makeBtnV900(archived?'恢復':'結案',archived?'btn green':'btn',()=>window.archiveSurveyV775(s.id,!archived)));
      if(canDeleteSurveyV900(s))group.appendChild(makeBtnV900('刪除','btn red',()=>showDeleteSurveyModalV776(s.id)));
      if(!current&&!archived)group.appendChild(makeBtnV900('設目前','btn green currentActionV787',()=>setActiveSurvey(s.id)));
      ops.innerHTML='';
      ops.appendChild(group);
    });
  }
