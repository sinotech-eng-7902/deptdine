...
let D={departments:[],members:[],memberAccounts:[],surveys:[],dates:[],restaurants:[],responses:[],final:null,managers:[],budgetEligibility:[],frontProtection:null,diningAssignments:[]};
...
const DESCRIPTION_TEXT_COLORS={default:'#334155',brand:'#1d4ed8',strong:'#172033',alert:'#b42318',teal:'#0f766e'};
function descriptionColorToken(value){
  let raw=String(value||'').trim().toLowerCase().replace(/\s+/g,'');
  let colors={
    '#334155':'default','rgb(51,65,85)':'default',
    '#1d4ed8':'brand','rgb(29,78,216)':'brand',
    '#172033':'strong','rgb(23,32,51)':'strong',
    '#b42318':'alert','rgb(180,35,24)':'alert',
    '#0f766e':'teal','rgb(15,118,110)':'teal'
  };
  return colors[raw]||'';
}
function sanitizeRichHtml(value){
  let template=document.createElement('template');template.innerHTML=String(value||'');
  let allowed=new Set(['B','STRONG','UL','OL','LI','A','BR','DIV','P','SPAN']);
  [...template.content.querySelectorAll('*')].forEach(node=>{
    if(['SCRIPT','STYLE','IFRAME','OBJECT','EMBED','SVG','MATH'].includes(node.tagName)){node.remove();return}
    let colorToken=node.tagName==='SPAN'?(String(node.getAttribute('data-text-color')||'').trim()||descriptionColorToken(node.style.color)):'';
    ...
    if(node.tagName==='SPAN'){
      if(Object.prototype.hasOwnProperty.call(DESCRIPTION_TEXT_COLORS,colorToken))node.setAttribute('data-text-color',colorToken);
...
function ensureRichDescriptionEditor(){
  ...
  let colorSelect=el('svDescTextColor');
  colorSelect?.addEventListener('mousedown',rememberRichDescriptionSelection);
  colorSelect?.addEventListener('change',()=>{let token=colorSelect.value;colorSelect.value='';if(token)runRichTextColor(token)});
}
let richDescriptionSelection=null;
function rememberRichDescriptionSelection(){
  let editor=richEditor(),selection=window.getSelection();
  if(editor&&selection?.rangeCount&&editor.contains(selection.anchorNode)&&!selection.isCollapsed)richDescriptionSelection=selection.getRangeAt(0).cloneRange();
}
function restoreRichDescriptionSelection(){
  let editor=richEditor(),selection=window.getSelection();
  if(selection?.rangeCount&&editor?.contains(selection.anchorNode)&&!selection.isCollapsed)return selection;
  if(!editor||!richDescriptionSelection)return null;
  try{selection.removeAllRanges();selection.addRange(richDescriptionSelection);return selection}catch(e){return null}
}
function runRichTextColor(token){
  let editor=richEditor(),color=DESCRIPTION_TEXT_COLORS[token];if(!editor||!color)return;
  let selection=restoreRichDescriptionSelection();
  if(!selection?.rangeCount||selection.isCollapsed||!editor.contains(selection.anchorNode)){editor.focus();return alert('請先選取要變更顏色的說明文字')}
  document.execCommand('styleWithCSS',false,true);
  document.execCommand('foreColor',false,color);
  document.execCommand('styleWithCSS',false,false);
  editor.innerHTML=sanitizeRichHtml(editor.innerHTML);
  richDescriptionSelection=null;syncRichDescription(true);editor.focus();
}
function runRichCommand(command){
  ...
  if(command==='createLink'){
    ...
    document.execCommand('styleWithCSS',false,true);document.execCommand('foreColor',false,DESCRIPTION_TEXT_COLORS.default);document.execCommand('styleWithCSS',false,false);
  }else document.execCommand(command,false,null);
  editor.innerHTML=sanitizeRichHtml(editor.innerHTML);richDescriptionSelection=null;syncRichDescription(true);editor.focus();
}
...
async function loadAll(){
  ...
  try{
    const diningAssignmentDoc=await doc('systemSettings','diningAssignments').get();
    D.diningAssignments=diningAssignmentDoc.exists&&Array.isArray(diningAssignmentDoc.data()?.items)?diningAssignmentDoc.data().items:[];
  }catch(e){
    console.warn('讀取聚餐負責人安排失敗',e);D.diningAssignments=[];
  }
  try{
...
function weightedScoreFormulaV1039(stat){let choices=Array.isArray(stat?.counts)?stat.counts:[],choiceCount=choices.length;let terms=choices.map((votes,index)=>rankLabel(index)+'選擇 '+votes+' 票 × '+(choiceCount-index)+' 分');return(terms.length?terms.join('＋')+'＝'+Number(stat?.score||0)+' 分':'目前沒有可計分的餐廳選擇')+'；不克參加者不納入計分。'}
  ...
  resultTables.innerHTML=
    ...
    <section class="resultGroup resultAnalysisGroup">
      ...
      <div class="resultGrid">
        ...
        <section class="resultSection"><div class="resultSectionHead"><h3>餐廳排名</h3></div><div class="restaurantRanking">${restStats.map((x,i)=>{let scoreFormula=weightedScoreFormulaV1039(x);return`<div class="rankResult"><span class="rankIndex">${i+1}</span><div class="rankResultMain"><h4>${esc(x.name)}</h4><div class="rankChoiceMetrics">${x.counts.map((count,rankIndex)=>`<span><small>${rankLabel(rankIndex)}選擇</small><b>${count}</b></span>`).join('')||'<span class="muted">尚無選擇資料</span>'}</div></div><div class="rankScore"><strong>${x.score}</strong><span class="rankScoreLabel">加權分數<button class="scoreHelp" type="button" aria-label="${escAttr('加權分數計算：'+scoreFormula)}" data-tooltip="${escAttr(scoreFormula)}">?</button></span></div></div>`}).join('')||'<div class="muted">尚無餐廳資料</div>'}</div></section>
      </div>
    </section>
      ...
      <section class="resultSection responseResultsSection resultDataSectionV953">
        ...
        <div class="resultDetailsBody">
          <div class="resultTools">
            ...
            <select id="responseAttendanceFilter" aria-label="出席狀態" onchange="filterResponseRows()"><option value="">全部狀態</option><option value="attend">可參加</option><option value="cannot">不克參加</option></select>
...
function responseAttendanceMatchesV1041(filter,cannotAttend){if(filter==='attend')return!cannotAttend;if(filter==='cannot')return!!cannotAttend;return true}
function filterResponseRows(){
  ...
  let attendance=document.getElementById('responseAttendanceFilter')?.value||'';
...
async function saveFinal(){
  const handler=window.saveFinalDecisionV1033;
  if(typeof handler!=='function')return alert('最終決議功能尚未完成載入，請重新整理頁面後再試一次。');
  return handler();
}
...
function installV711AdminUI(){
  ...
  if(side&&!document.getElementById('logNavButton')){const button=document.createElement('button');button.id='logNavButton';button.className='nav';button.dataset.access='system';button.textContent='操作紀錄';button.onclick=()=>panel('logP',button);const excel=[...side.querySelectorAll('.nav')].find(x=>x.textContent.trim()==='資料匯出');side.insertBefore(button,excel||null)}
