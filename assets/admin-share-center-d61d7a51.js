/* v10.11 activity share center: local QR generation and link actions. */
(function(){
  'use strict';

  const MODULE_VERSION='10.11';
  ...
  function renderQr(url){
    ...
    host.innerHTML='';error.textContent='';
  ...
  function downloadQrImage(){
    ...
    const source=document.querySelector('#shareQrCodeV1011 canvas');
  ...
  function installMenu(){
    const menu=document.querySelector('#adminMoreMenu .adminMoreMenuPanel');
    if(!menu)return;
    const oldAdmin=document.getElementById('copyAdminLinkBtn'),oldFront=document.getElementById('copyFrontLinkBtn');
    if(oldAdmin)oldAdmin.hidden=true;
    if(oldFront)oldFront.remove();
    let button=document.getElementById('shareActivityBtnV1011');
    if(!button){
      button=document.createElement('button');button.id='shareActivityBtnV1011';button.className='btn';button.type='button';button.textContent='分享活動';
      button.addEventListener('click',()=>{const details=document.getElementById('adminMoreMenu');if(details)details.open=false;open()});
      menu.prepend(button);
    }
  }
  ...
  window.AdminShareCenter=Object.freeze({version:MODULE_VERSION,open,close,installMenu});
})();
